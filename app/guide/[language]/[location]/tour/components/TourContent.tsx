...
const DialogueContent: React.FC<DialogueContentProps> = ({ locationName, language }) => {
  ...
  return (
    <div className="space-y-3 mt-4">
      <div className="text-sm font-medium text-gray-900 mb-3">
        💬 진행자와 게스트의 대화 ({segments.length}개)
      </div>
        ...
        {segments.map((segment, index) => (
          <div
            ...
            className={`p-3 rounded-lg border-l-4 ${
              segment.speakerType === 'male' 
                ? 'border-blue-400' // 진행자: 파란색 테두리
                : 'bg-gray-50 border-gray-400' // 게스트: 회색 배경 + 회색 테두리
            }`}
          >
            <div className="flex items-center justify-between mb-2">
              <span className={`text-xs font-medium px-2 py-1 rounded-full ${
                ...
                {segment.speakerType === 'male' ? '진행자' : '게스트'}
              </span>
              <span className="text-xs text-gray-500">
                {Math.floor(segment.duration / 60)}:{String(Math.floor(segment.duration % 60)).padStart(2, '0')}
              </span>
            </div>
                        ...
                        {expandedChapters.includes(index) && (
                          <div className="border-t border-border p-2">
                            <Stack space="sm">
                              ...
                              {index === 0 && (
                                <DialogueContent
                                  locationName={guide?.metadata?.originalLocationName || guide?.overview?.title || ''}
