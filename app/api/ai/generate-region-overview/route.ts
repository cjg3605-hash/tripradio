...
export async function POST(request: NextRequest) {
  try {
    ...
    // 위치 정보 확인 (좌표 등 기본 정보)
    const locationResult = await routeLocationQueryCached(sanitizedLocation);
    console.log('📍 위치 분류 결과:', locationResult);
    ...
    // 국가인지 지역/도시인지 판단
    const isCountryLevel = Boolean(locationResult.locationData && locationResult.locationData.level <= 1); // Level 0-1은 국가
    
    console.log(`🎯 위치 분류: ${sanitizedLocation} → Level ${locationResult.locationData?.level} → ${isCountryLevel ? '국가 (도시 추천)' : '지역/도시 (관광지 추천)'}`);
