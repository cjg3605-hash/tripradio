...
async function verifyDDPPodcast() {
  console.log('🔍 DDP 팟캐스트 검증 시작...\n');

  const { data: episodes } = await supabase
    .from('podcast_episodes')
    .select('*')
    .eq('location_slug', 'dongdaemun-design-plaza')
    .eq('language', 'ko')
    .order('created_at', { ascending: false })
    .limit(1);

  if (!episodes || episodes.length === 0) {
    console.log('❌ DDP 에피소드 없음');
    return;
  }
  ...
  console.log('\n📚 챕터별 분석:');
  Object.keys(chapters).sort((a, b) => parseInt(a) - parseInt(b)).forEach(chIdx => {
    const ch = chapters[chIdx];
    const firstSpeaker = ch[0].speaker_type;
    const lastSpeaker = ch[ch.length - 1].speaker_type;
    console.log(`\n챕터 ${parseInt(chIdx)}: ${episode.chapter_timestamps?.[chIdx]?.title || 'N/A'}`);
  ...
  // 스크립트 샘플
  console.log('📝 스크립트 샘플 (처음 10개 세그먼트):\n');
  segments.slice(0, 10).forEach((seg, idx) => {
    const icon = seg.speaker_type === 'male' ? '👨' : '👩';
    console.log(`${idx + 1}. ${icon} [${seg.speaker_type}] ${seg.text_content.substring(0, 80)}...`);
  });
}

verifyDDPPodcast().catch(console.error);
