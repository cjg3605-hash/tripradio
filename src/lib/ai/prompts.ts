...
- **챕터 1 이후:** 추천 경로 단계를 따라가며 각 특정 장소에 대한 상세한 해설을 제공해야 합니다.`
    ...
    en: {
      ...
      outputInstructions: 'You must strictly return only a single, pure JSON object by following these rules:\n- Do not include any text outside the JSON object, such as introductions, notes, or markdown code blocks (```).\n- Adhere 100% to JSON syntax.\n- The JSON structure and key names must be identical to the example below. Do not translate or change key names.\n- **Any JSON syntax error is a critical failure.**\n- Example of the final output structure:\n' + MINIMAL_EXAMPLE_JSON + '\n\n**IMPORTANT:** For every item in route.steps, you MUST generate a corresponding chapter in realTimeGuide.chapters. The number and order of chapters in realTimeGuide.chapters MUST match route.steps exactly. Each chapter must include title, location, humanStories, coreNarrative, nextDirection, and sceneDescription fields, and the title/location must match the corresponding step.\n\n**Every field (humanStories, coreNarrative, nextDirection, sceneDescription, etc.) in every chapter must be filled with real, substantial content. Never return empty ("") or placeholder chapters. Chapters with missing or empty content are strictly forbidden.',
