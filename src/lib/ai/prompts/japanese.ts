...
const AUDIO_GUIDE_EXAMPLE = {
  content: {
    ...
    route: {
      steps: [
        { step: 1, location: "入口", title: "メイン入口" },
        { step: 2, location: "中央ホール", title: "中央ホール" },
        { step: 3, location: "主要展示", title: "主要展示" },
        { step: 4, location: "特別空間", title: "特別空間" },
        { step: 5, location: "展望台", title: "展望台" }
      ]
    },
    realTimeGuide: {
      chapters: [
        {
          ...
          // coordinatesフィールドはシステムが自動で追加 - AIは記述しない
        }
      ]
    }
  }
...
### 6. **📍 座標情報 - システム自動処理**
**🚨 重要: AIは座標を直接生成しないでください！**
- **座標はシステムで精密に自動追加されます**
- **AIはnarrative、title、nextDirectionフィールドのみを生成してください**
- **coordinatesフィールドはシステムの正確な座標検索システムが処理します**

### 📏 **AI執筆範囲（座標除く）**
- **title**: 具体的な場所名のみ（コロンなし）
- **narrative**: 詳細なオーディオガイド内容
- **nextDirection**: 次の場所への移動案内
- **🚨 coordinatesフィールドは絶対に記述しないでください - システムが自動処理します**
...
\`\`\`json
{
  "route": {
    "steps": [
      { "step": 1, "location": "入口", "title": "入口" },
      { "step": 2, "location": "核心エリア", "title": "中心部: 가장 중요한 볼거리" },
      { "step": 3, "location": "特別空間", "title": "特別展示空間: 숨겨진 보석 같은 장소" }
    ]
  },
  "realTimeGuide": {
    "chapters": [
      { "id": 0, "title": "[場所名] [具体的な開始地点]" },
      { "id": 1, "title": "中心部" },
      { "id": 2, "title": "特別展示空間" }
    ]
  }
