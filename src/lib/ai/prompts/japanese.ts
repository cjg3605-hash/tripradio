  ...
  content: {
    ...
    realTimeGuide: {
      chapters: [
        {
          id: 0,
          title: "${locationName}の実際の観光開始地点を見つけて、次の形式で記述してください：「[実在する施設名] [具体的な場所]: ${locationName}観光出発点」\n\n✅ 正しい例：\n- 「清水寺仁王門チケット売場: 清水寺観光出発点」\n- 「東京タワー大展望台入口: 東京タワー観光出発点」\n- 「富士山5合目バスターミナル: 富士山観光出発点」\n\n🚨 必ずGoogle Mapsで検索可能な実在する具体的施設名を使用してください",
          ...
          coordinates: {
            lat: "[上記titleで指定した実際の施設の位置を知識の範囲で推定した緯度 - 不明な場合は0.0]",
            lng: "[上記titleで指定した実際の施設の位置を知識の範囲で推定した経度 - 不明な場合は0.0]",
            description: "[該当施設の具体的な位置説明（システムで精密補正されます）]"
          }
        }
      ]
    }
