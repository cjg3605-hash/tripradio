...
- **coordinates**: 各チャプターの実際の場所の正確なGPS座標（lat、lng、description）
- **形式**: { "lat": 34.9949, "lng": 135.7851, "description": "清水寺本堂清水の舞台" }
- **精度**: 公式ソースから確認された正確な座標を使用
- **説明**: チャプター内容と一致する簡潔な場所説明

### 7. **🔍 厳格な事実検証原則**
すべての情報は検証可能で正確でなければなりません：
- **歴史的事実**: 信頼できるソースからの検証済み情報のみ
- **数値と日付**: 正確な測定値、建設年代、歴史的年代順
- **推測の禁止**: 未確認の伝説、仮定、解釈的内容を避ける
- **公式ソース**: 公式ガイド、学術ソースで入手可能な情報を参照
- **事実確認必須**: すべての歴史的主張は検証可能でなければならない`,
...
### 8. **📋 概要セクションの厳格な制限**
**背景フィールドの文字数制限：**
- **小規模な場所（店舗、小さな建物）**: 最大300-400文字
- **中規模な場所（寺院、博物館）**: 最大500-600文字
- **複雑な場所（宮殿、大規模複合施設）**: 最大700-800文字
- **禁止事項**: モバイルUIオーバーフローを引き起こす過度の長さ

### 9. **🎯 必見スポット標準化**
**形式**: #スポット名1 #スポット名2 #スポット名3 #スポット名4 #スポット名5
- **正確に5つのスポット**: それ以上でもそれ以下でもない
- **具体的な名称**: 一般的な説明ではなく、実際の建物/エリア名を使用
- **一般的な用語を避ける**: "本堂"、"中央エリア"ではなく、固有名詞を使用
- **ハッシュタグ形式**: 各スポットは#記号で始まる必要がある
- **スペース区切り**: 各#タグ付きスポット間は単一スペース

### 10. **📜 イントロチャプター要件 - 重要！**
**最初のチャプター（ID=0）は包括的な紹介（1200-1500文字）でなければなりません：**
- **歴史的背景と変遷（400文字）**: 創建/設立背景、元々の用途と現在の用途の変化、重要な歴史的事件と転換点、時代別変化と拡張過程
- **建築と空間構成の哲学（300文字）**: 設計意図と建築様式の意味、空間配置の目的と動線計画、象徴的要素とその意味
- **観覧戦略と核心ポイント（300文字）**: 全体区域概要と推奨観覧順序、時間帯別観覧のコツ、見逃しやすい隠れた宝物、各チャプターで特に注目すべき要素
- **現地人視点の特別な物語（200文字）**: 一般観光客が知らない興味深い事実、現地文化との関連性、季節別/時間別の異なる魅力、訪問時に知っておくと便利な実用情報
- **重要**: マークダウン書式（**テキスト**、##見出し、📜絵文字）を使わずプレーンテキストで作成
