...
📝 작성 요구사항
1. 분량: narrative 필드에 반드시 1700자 이상의 풍부한 내용
...

{
  "chapter": {
    "id": ${chapterIndex},
    "title": "${chapterTitle}",
    "narrative": "이 부분에 반드시 1700자 이상의 상세하고 흥미진진한 오디오 가이드 스크립트를 작성하세요. 현장 묘사부터 시작해서 역사적 배경, 관련 인물들의 이야기, 그리고 다음 장소로의 자연스러운 연결까지 하나의 연속된 이야기로 만들어주세요. 실제 관광 가이드가 현장에서 관광객들에게 들려주는 것처럼 구어체로 친근하게 작성하고, 중간중간 '자, 여기 보세요', '정말 놀라운 건', '여러분이 지금 보고 계신' 같은 상호작용적 표현을 넣어서 생동감 있게 만들어주세요.",
...
- narrative 필드는 최소 1700자 (실제 측정 필요)
- 서론이나 설명 없이 바로 JSON 시작
- 코드블록 표시 절대 금지
- 문법적으로 완벽한 JSON 형식
