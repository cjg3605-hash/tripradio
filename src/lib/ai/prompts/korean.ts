...
**중요:** 각 챕터의 내용은 반드시 아래 순서로 자연스럽게 이어지게 작성하세요.
1. 장면 설명(sceneDescription)
2. 인간 이야기(humanStories)
3. 핵심 이야기(coreNarrative)
이 순서가 지켜지지 않으면 시스템 오류가 발생합니다. 각 필드는 구분 없이 하나의 오디오 스크립트처럼 자연스럽게 연결하세요.
**"장면 설명", "핵심 이야기", "인간 이야기"라는 제목(문구)은 절대 결과물에 포함하지 마세요.**
각 필드는 구분 없이 자연스럽게 연결된 오디오 스크립트처럼 작성하세요.
