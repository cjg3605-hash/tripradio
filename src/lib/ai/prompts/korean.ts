...
🚨 **핵심: JSON 구조 필수 요구사항**:
- realTimeGuide는 반드시 객체여야 합니다 (배열 아님)
- realTimeGuide.chapters는 반드시 챕터 객체들의 배열이어야 합니다
- realTimeGuide 자체를 배열로 만들면 절대 안됩니다
...
- ✅ **필수: realTimeGuide는 객체, chapters는 배열**`;
...
🔥 **분량 1500자 미달 시 재생성 필요** 🔥
- 짧은 답변, 요약형 답변 절대 금지
- 반드시 풍부하고 상세한 완전한 가이드 제공
- AI는 분량을 최우선 목표로 설정하여 응답

지금 바로 "${chapterTitle}" 챕터의 **완전하고 풍부한 1500-1600자 분량** 오디오 가이드를 생성해주세요!`;
}
