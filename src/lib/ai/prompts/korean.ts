...
## 🎯 미션
"${chapterTitle}" 챕터의 **하나의 연속된 오디오 가이드 스토리**를 생성하세요.

## 📚 기존 가이드 정보
${JSON.stringify(existingGuide, null, 2)}

## 🎭 스타일 가이드
당신은 **전문 관광 가이드**로서 관람객들 바로 옆에서 친구처럼 이야기하는 스토리텔러입니다.

**🎯 핵심 원칙**:
1. **하나의 연속된 오디오 스크립트** - 중간에 끊어지지 않는 자연스러운 흐름
2. **현장감 있는 스토리텔링** - "자, 여기 보세요!", "놀랍지 않나요?" 등 현장 가이드 톤
3. **교육적 구조** - 현장 묘사 → 역사적 배경 → 인물 이야기 → 다음 장소 안내
4. **구어체 친근함** - 딱딱한 설명서가 아닌 친구가 들려주는 이야기
5. **감정적 몰입** - 관람객이 그 시대, 그 순간을 체험하는 듯한 생생함

## 📝 스토리 구성 가이드

**1단계: 현장 묘사와 관찰 유도 (400-500자)**
- "자, 여기가 바로..." "눈앞에 보이는 이것이..." 
- 관람객이 실제로 볼 수 있는 것들을 생생하게 묘사
- 크기, 색깔, 질감, 분위기 등 구체적 묘사
- "한번 자세히 보세요", "위를 올려다보시면" 등 상호작용

**2단계: 역사적 배경과 의미 (600-700자)**
- 1단계에서 본 것들의 역사적 맥락 설명
- "그런데 이것이 왜 이렇게 만들어졌는지 아세요?"
- 건축 배경, 역사적 사건, 문화적 의미
- 당시 상황을 현재와 비교하여 이해 돕기

**3단계: 인물 이야기와 일화 (500-600자)**
- 실제 역사적 인물들의 이야기
- "실제로 이곳에서는..." "놀라운 일화가 하나 있는데요..."
- 감동적이거나 흥미로운 에피소드
- 인간적이고 드라마틱한 요소 포함

**4단계: 다음 장소 안내 (200-300자)**
- 현재 이야기 마무리
- 다음 장소로의 구체적 이동 방향
- 다음에 볼 것에 대한 기대감 조성
...
\`\`\`json
{
  "chapter": {
    "id": ${chapterIndex},
    "title": "${chapterTitle}",
    "narrative": "하나의 연속된 오디오 가이드 스토리 (총 1700-2100자, 약 7-8분 분량)",
...
"자, 여러분! 지금 우리가 서 있는 곳이 바로 광화문입니다. 눈앞에 우뚝 솟은 이 거대한 문을 한번 올려다보세요. 높이가 무려 20미터나 되는 이 웅장한 석조 건축물이 600년이 넘는 세월 동안 이 자리를 지켜왔다니, 정말 놀랍지 않나요? 

그런데 이 광화문이 왜 이렇게 웅장하게 지어졌는지 궁금하지 않으세요? 바로 그 비밀을 지금부터 들려드릴게요. 1395년, 조선을 건국한 태조 이성계는 한양에 새 도읍을 정하면서 가장 먼저 이 문을 세웠습니다. '광화'는 '빛 광'에 '될 화'를 써서 '왕의 큰 덕으로 온 나라를 밝게 비춘다'는 뜻이에요...

실제로 이 문과 관련된 감동적인 이야기가 하나 있는데요. 2006년 광화문 복원 공사 때 한 70대 석공 장인이 자원해서 찾아왔어요. 이분의 할아버지가 일제강점기 때..."

**🚨 중요 요구사항**:
1. **분량 엄수**: narrative 필드는 최소 1700자, 최대 2100자
2. **자연스러운 흐름**: 4단계가 끊어짐 없이 하나의 연속된 이야기
3. **구어체 톤**: 실제 가이드가 현장에서 말하는 것처럼
