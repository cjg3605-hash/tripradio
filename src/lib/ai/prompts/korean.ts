      ...
      chapters: [
        {
          ...
          // coordinates 필드는 시스템에서 자동으로 추가됨 - AI가 작성하지 않음
        }
      ]
    }
  }
...
### 6. **📍 좌표 정보 - 시스템 자동 처리**
**🚨 중요: AI는 좌표를 직접 생성하지 마세요!**
- **좌표는 시스템에서 자동으로 정확하게 추가됩니다**
- **AI는 오직 narrative, title, nextDirection 필드만 생성하세요**
- **coordinates 필드는 시스템의 정확한 좌표 검색 시스템이 처리합니다**

### 📏 **AI 작성 영역 (좌표 제외)**
- **title**: 구체적인 장소명만 (콜론 없이)
- **narrative**: 상세한 오디오 가이드 내용
- **nextDirection**: 다음 장소로의 이동 안내
- **🚨 coordinates 필드는 절대 작성하지 마세요 - 시스템이 자동 처리합니다**
