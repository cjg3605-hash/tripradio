  ...
  content: {
    ...
    realTimeGuide: {
      chapters: [
        {
          id: 0,
          title: "${locationName}의 실제 관광 시작지점을 찾아 다음 형식으로 작성하세요: '[실제 존재하는 시설명] [구체적 위치]: ${locationName} 관광 시작점'\n\n✅ 올바른 예시:\n- '경복궁 광화문 매표소: 경복궁 관광 시작점'\n- '에펠탑 트로카데로 지하철역 출구: 에펠탑 관광 시작점'\n- '후지산 5합목 버스터미널: 후지산 관광 시작점'\n\n🚨 반드시 실제 존재하는 구체적 시설명을 사용하세요 (Google 지도에서 검색 가능한 실제 장소)",
...
**🏛️ 박물관/미술관:**
- "국립중앙박물관 매표소: 한국사 여행의 출발점"
- "루브르 박물관 피라미드 입구: 예술 순례의 시작"

**🏰 궁궐/성/유적:**
- "경복궁 광화문 매표소: 조선 왕조 500년사의 시작점"
- "베르사유 궁전 방문자센터: 프랑스 절대왕정의 입구"

**🏔️ 자연공원/명산:**
- "남산케이블카 승강장: 서울 전망 여행의 시작"
- "후지산 5합목 버스정류장: 영산 등반의 베이스캠프"

**🚇 교통중심 시작점:**
- "명동역 2번 출구: 명동성당 순례의 시작점"
- "로마 콜로세움역 출구: 고대 로마 체험의 관문"

### 🎯 **인트로 제목 생성 공식:**
\`\`\`
"[구체적 시설명 + 접근점]: [해당 관광지 전체를 관람하기 위한 시작점임을 명시]"
\`\`\`

**🔴 필수 요구사항:**
1. **매표소, 안내소, 방문자센터, 역출구, 승강장** 등 실제 존재하는 시설명 사용
2. **관광객이 Google 지도나 길찾기로 실제 찾아갈 수 있는 장소**
3. **해당 관광지 관람을 위한 논리적 시작점**
4. **"메인 입구", "시작점" 같은 애매한 표현 절대 금지**
