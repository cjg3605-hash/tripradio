...
const AUDIO_GUIDE_EXAMPLE = {
  content: {
    overview: {
      title: "경복궁",
      location: "서울특별시 종로구",
      keyFeatures: "조선 왕조의 법궁으로, 웅장하고 아름다운 궁궐 건축 양식",
      background: "태조 이성계가 한양에 도읍을 정하면서 창건한 조선의 법궁이며, 임진왜란 때 소실되었다가 흥선대원군에 의해 중건",
...
4. **풍부한 내용 (각 필드별 분량)**
   - sceneDescription: 700-800자 (약 2분 30초) - 배경지식 + 현장 관찰(사실반영)
   - coreNarrative: 500-600자 (약 2분) - 역사적 맥락과 의미  
   - humanStories: 200-300자 (약 1분) - 실제 인물 이야기
   - nextDirection: 100-200자 (약 40초) - 이동 안내만
...
### 2. **개요(overview) 구조 - 새로운 양식**
개요는 다음 3개 필드로 구성되어야 합니다:
- **location**: 정확한 위치 (예: "서울특별시 종로구")
- **keyFeatures**: 주요 특징 (예: "조선 왕조의 법궁으로, 웅장하고 아름다운 궁궐 건축 양식")
- **background**: 만들어진 배경 (예: "태조 이성계가 한양에 도읍을 정하면서 창건한 조선의 법궁이며, 임진왜란 때 소실되었다가 흥선대원군에 의해 중건")
...
### 4. **최소 분량 (한국어 기준)**
- sceneDescription: 700자 이상 (현장 생생한 묘사)
- coreNarrative: 500자 이상 (역사적 사실과 의미)
- humanStories: 200자 이상 (핵심 인물 이야기)
- nextDirection: 100자 이상 (구체적 경로 안내)
