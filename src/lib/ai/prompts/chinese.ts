...
为${locationName}提供具有深度专业知识的最高品质中文导游解说。
...
### 1. **纯净JSON格式**
- 无前言、说明、代码块(\`\`\`)，仅返回有效JSON
- 完美JSON语法遵循（逗号、引号、括号）
- 键名与示例100%相同（禁止翻译）
- **禁用表情符号**：📍 ✨ 🏛️ 🎯 等所有表情符号，仅使用纯文本

### 🚀 **质量提升核心原则**
- **专业性**: ${typeConfig?.expertRole || '综合专家'}水平的深度与洞察
- **🎯 事实性**: 仅使用可验证的具体事实和测量值 - 以官方记录、学术资料为准
- **准确性**: 所有信息基于确认事实，绝对禁止推测或假设
- **独创性**: 强调使此地与其他地方区别开来的特色要素
- **故事叙述**: 基于事实的引人入胜叙事，而非枯燥信息

### 🔍 **${locationType.toUpperCase()} 质量验证标准**
${getQualityRequirementsByType(locationType)}

### 🚨 **绝对禁止事项**
- **泛泛表达**: "请想象"、"令人惊讶"、"体验一下"、"让我们稍作休息"
- **模糊指示**: "这里"、"这个地方"（必须使用具体地名）
- **无法验证内容**: 推测、假设、个人意见、未确认信息
- **夸张表达**: 无根据的"世界最高"、"史无前例"、"令人难以置信"等修饰语
- **不确定表达**: "也许"、"推测"、"传说中"、"据说"等
- **虚构内容**: 虚构人物、编造轶事、推测性情节
- **空泛信息**: 没有实质信息仅填充空间的内容
- **🔥 重复信息**: 在多个章节重复相同历史背景、建立·修复年代

### 2. **简洁概览结构**
概览应由以下3个字段组成:
- **location**: 仅地区名，15字符以内（例："北京市东城区"）
- **keyFeatures**: 核心关键词，25字符以内（例："明清皇宫建筑群"）
- **background**: 一行总结，30字符以内（例："1420年建成的皇家宫殿"）
...
- **第1章节以后**: 各个独立空间·建筑独有的特征、功能、特别故事

#### ❌ **绝对禁止 - 重复模式**
错误示例:
• 介绍: "紫禁城太和殿的建筑特征说明" ← 🚫 以个别建筑为中心！
• 第1章: "紫禁城建于1420年，经历战乱后重建" ← 🚫 应在介绍中处理的内容！
• 第2章: "乾清宫与1420年紫禁城建设一起..." ← 🚫 重复！

#### ✅ **正确示例**
• 介绍: "紫禁城整体介绍 - 从1420年建设到现在的综合背景" ← ✅ 综合背景！
• 第1章: "太和殿独特的建筑样式和斗拱工艺的技术意义" ← ✅ 个别特征！
• 第2章: "乾清宫的三进构造和皇帝私人生活的空间" ← ✅ 固有特性！

### 6. **完整音频导游结构**
- overview: 地点整体概览（简洁格式）
  ...
  "content": {
    ...
    "realTimeGuide": {
      "chapters": [
        {
          "id": 0,
          "title": "找到${locationName}的真实旅游起点，按以下格式书写：'[真实存在的设施名称] [具体位置]: ${locationName}游览起点'\n\n✅ 正确示例：\n- '故宫博物院午门售票处: 故宫博物院游览起点'\n- '埃菲尔铁塔特罗卡德罗地铁站出口: 埃菲尔铁塔游览起点'\n- '富士山五合目巴士总站: 富士山游览起点'\n\n🚨 必须使用真实存在的具体设施名称（在Google地图上可搜索到的实际地点）",
          ...
          "coordinates": {
            "lat": "[在知识范围内估算上述title中提到的真实设施位置纬度 - 不知道则用0.0]",
            "lng": "[在知识范围内估算上述title中提到的真实设施位置经度 - 不知道则用0.0]",
            "description": "[该设施的具体位置说明 (系统将进行精确校正)]"
          }
        }
      ]
...
**🚨 最终确认事项:**
- 所有章节标题都必须是"地点名: 简洁说明"格式
- 地点名必须是实际具体的地点名称
- 不要用一般性描述或动作表达来制作标题
- **绝对重要**: 全部内容必须使用中文编写，不得出现韩语字符`;
...
` : '';
  ...
  const prompt = `# 🎙️ "${locationName}" 沉浸式中文音频导游生成任务
...
**现在就请为"${locationName}"生成自然迷人的中文音频导游，使用纯净JSON格式！**`;
