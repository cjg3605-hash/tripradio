...
### 6. **📍 GPS坐标必须 - 非常重要！**
每个章节必须包含准确的GPS坐标：
- **coordinates**: 每个章节实际位置的精确GPS坐标（lat、lng、description）
- **格式**: { "lat": 39.9163, "lng": 116.3972, "description": "故宫午门主入口" }
- **精度**: 使用从官方来源验证的精确坐标
- **说明**: 与章节内容匹配的简要位置说明
...
所有信息必须可验证且准确：
- **历史事实**: 仅来自可靠来源的已验证信息
- **数字和日期**: 精确的测量值、建设年代、历史年表
- **禁止推测**: 避免未确认的传说、假设、解释性内容
- **官方来源**: 引用官方指南、学术来源中可获得的信息
- **事实核查必需**: 所有历史声明必须可验证`,
...
### 8. **📋 概述部分严格限制**
**背景字段字符限制：**
- **小型场所（商店、小建筑）**: 最多300-400字符
- **中型场所（寺庙、博物馆）**: 最多500-600字符
- **复杂场所（宫殿、大型综合体）**: 最多700-800字符
- **禁止事项**: 造成手机界面溢出的过度长度

### 9. **🎯 必游景点标准化**
**格式**: #景点名1 #景点名2 #景点名3 #景点名4 #景点名5
- **正好5个景点**: 不多不少
- **具体名称**: 使用实际建筑/区域名称，不要一般性描述
- **避免通用词汇**: 避免"大殿"、"中央区域" - 使用专有名词
- **井号格式**: 每个景点必须以#符号开始
- **空格分隔**: 每个#标记景点之间用单个空格

### 10. **📜 介绍章节要求 - 重要！**
**第一个章节（ID=0）必须是全面介绍（1200-1500字符）：**
- **历史背景与演变（400字符）**: 创建/设立背景，原始用途与现在用途的变化，重要历史事件和转折点，时代变化和扩展过程
- **建筑与空间构成哲学（300字符）**: 设计意图和建筑风格含义，空间布局目的和流线规划，象征性元素及其意义
- **参观策略与核心要点（300字符）**: 整体区域概览和推荐参观顺序，时段性参观技巧，容易错过的隐藏瑰宝，各章节需特别关注的要素
- **当地人视角的特别故事（200字符）**: 一般游客不知道的有趣事实，与当地文化的关联性，季节/时间不同魅力，访问时有用的实用信息
- **重要**: 使用纯文本编写，不使用markdown格式（**文本**、##标题、📜表情符号）
