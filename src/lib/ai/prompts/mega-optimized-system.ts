    ...
    "chapters": [
      {
        ...
        "narrative": "🎯 인트로 챕터 특수 생성 규칙 적용:\n1. 감정적 몰입 유도 오프닝 (${Math.round(expert.verified_patterns.emotional_engagement * 100)}%)\n2. ${expert.tone} 톤으로 문화적 맥락 설명\n3. ${expert.cultural_wisdom[0]}\n4. 건축/역사적 특징을 ${expert.verified_patterns.optimal_story_ratio * 100}% 스토리 비율로 서술\n5. 개인적 연결감과 기대감 조성\n6. 다음 여정에 대한 자연스러운 연결",
        "nextDirection": "첫 번째 핵심 공간으로의 이동 안내"
      }
...
**인트로 챕터는 전체 투어의 첫인상을 결정하는 핵심 요소입니다.**

### 필수 구성 요소 (${expert.expertise} 전문성 반영):
1. **감정적 몰입 오프닝**: 시간이 멈춘 듯한 순간/경외감을 느낄 수 있는 표현으로 시작
2. **문화적 맥락**: ${expert.tone} 톤으로 해당 지역/문명의 역사적 배경 설명
3. **건축/예술적 가치**: ${expert.cultural_wisdom[0]}를 적용한 전문적 해석
4. **구체적 사실**: 연도, 수치, 인명 등 검증 가능한 팩트 최소 3개 포함
5. **개인적 연결**: 방문자 각자의 발견과 감동 유도
6. **여정 예고**: 다음 챕터들에 대한 기대감 조성
