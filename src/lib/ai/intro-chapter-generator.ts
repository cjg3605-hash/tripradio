...
지금 ${locationData.name} 방문 팁을 작성하세요.
  ...
  private getFallbackHistoricalBackground(locationData: LocationData): string {
    return `${locationData.name}은 오랜 역사를 간직한 소중한 문화 공간입니다. 이곳은 시대의 변화 속에서도 그 가치와 의미를 지켜온 특별한 장소로, 방문객들에게 깊이 있는 역사적 경험을 선사합니다. 과거와 현재가 만나는 이 공간에서, 우리는 선조들의 지혜와 문화적 유산을 직접 체험할 수 있습니다.`;
  }
