...
interface SimulationScenario {
  ...
  realCoordinate: { lat: number; lng: number };
  nominatimResponse: {
    lat: string;
    lon: string;
    display_name: string;
...
const SIMULATION_SCENARIOS: SimulationScenario[] = [
  ...
  {
    name: "근거리 차이 (8m)",
    aiCoordinate: { lat: 37.5796, lng: 126.9770 },
    realCoordinate: { lat: 37.5808, lng: 126.9760 },
    nominatimResponse: {
      lat: "37.5808",
      lon: "126.9760",
  ...
  // 🟡 경계 케이스들 
  {
    name: "정확히 10m 차이 (경계값)",
    aiCoordinate: { lat: 37.5796, lng: 126.9770 },
    realCoordinate: { lat: 37.5806, lng: 126.9770 },
    nominatimResponse: {
      lat: "37.5806", 
      lon: "126.9770",
...

  {
    name: "10.1m 차이 (경계 초과)",
    aiCoordinate: { lat: 37.5796, lng: 126.9770 },
    realCoordinate: { lat: 37.5807, lng: 126.9770 },
    nominatimResponse: {
      lat: "37.5807",
      lon: "126.9770", 
    ...
    expectedBehavior: "2차 검증 진행 → Radar 결과 사용",
    potentialIssue: "미세한 차이로 불필요한 Radar 호출"
  },
...

  {
    ...
    aiCoordinate: { lat: 37.5796, lng: 126.9770 }, // 서울 경복궁
    realCoordinate: { lat: 37.5808, lng: 126.9760 },
    nominatimResponse: {
...
function suggestImprovements() {
  ...
  console.log("2. **신뢰도 임계값 조정**: 거리 10m + 신뢰도 0.8 이상일 때만 스킵");
  console.log("3. **도시 범위 체크**: 예상 도시 범위를 벗어나면 2차 검증 강제");
  console.log("4. **경계값 완화**: 10m → 5m로 더 엄격하게 적용");
  console.log("5. **A/B 테스트**: 실제 데이터로 정확도 비교 필요");
}

export { runImprovedSimulation, suggestImprovements, SIMULATION_SCENARIOS };