...
// 🔧 Leaflet 아이콘 안전한 초기화
if (typeof window !== 'undefined' && !window.leafletIconsInitialized) {
  try {
    ...
    console.log('🗺️ Leaflet 아이콘 초기화 완료');
  } catch (e) {
    console.warn('Leaflet 아이콘 초기화 경고:', e);
  }
}
...
const MapWithRoute = memo<MapWithRouteProps>(({ 
  ...
  // 🔧 안정적인 지도 키 생성 (early return 전에 위치)
  const mapKey = useMemo(() => {
    const locationKey = locationName?.replace(/[^a-zA-Z0-9가-힣]/g, '') || 'default';
    const coordsKey = center ? `${center.lat.toFixed(4)}_${center.lng.toFixed(4)}` : 'nocoords';
    const componentsKey = chapters ? 'chapters' : pois ? 'pois' : 'empty';
    const stableKey = `map-${locationKey}-${coordsKey}-${validChapters.length}-${componentsKey}`;
    
    console.log(`🗺️ 지도 키 생성: ${stableKey}`);
    return stableKey;
  }, [locationName, center, validChapters.length, chapters, pois]);
  ...
  return (
    ...
    >
      ...
      <MapErrorBoundary 
        ...
        validChaptersCount={validChapters.length}
      >
        <MapContainer 
          key={mapKey}
          center={mapCenter}
          ...
          preferCanvas={true}
          whenReady={() => {
            console.log(`🗺️ 지도 준비 완료! key: ${mapKey}, validChapters: ${validChapters.length}개`);
          }}
        >
          {/* 타일 레이어 */}
          <TileLayer
