...
async function verifyNamsanPodcast() {
  console.log('🔍 남산타워 팟캐스트 검증 시작...\n');

  // 1. 에피소드 조회
  const { data: episodes, error: episodeError } = await supabase
    .from('podcast_episodes')
    .select('*')
    .or('location_slug.eq.namsan-tower,location_input.ilike.%남산타워%')
    .eq('language', 'ko')
...
verifyNamsanPodcast().catch(console.error);
